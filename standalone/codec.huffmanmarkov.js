...
HuffmanMarkovCodec.prototype.decode = function(inText) {
    ...
    for (i = 0; i < len; i++) {
        ...
        // TODO: less sucky way of doing this
        // If nextState isn't a valid state, skip words until we re-synchronize
        if (this.decodingTrees[this.state][nextState] == null) continue;

        // Zoom through zero-bit words
        while (this.encodingTrees[this.state]["_"] != null) {
            this.state = this.encodingTrees[this.state]["_"];
        }


        // Replace each word with the binary value of its entry in the table (sans trailing "_")
        decoded[i] =
            this.decodingTrees[this.state][nextState].substring(1);

        // Move to the next state
        this.state = this.encodingTrees[this.state][this.decodingTrees[this.state][nextState]];
